...
###   This script attempts to download the signature file SHA256SUMS.asc from iranbit.org
...
WORKINGDIR="/tmp/iranbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://iranbit.org/bin/"
VERSIONPREFIX="iranbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'iranbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Iranbit Core binary release signing key installed?"
   fi
