Iranbit Core version 0.11.0 is now available from:

  <https://iranbit.org/bin/iranbit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Iranbit Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer IBT/kB fee to be
...
For example, add the following to `iranbit.conf`:
...
related to the blockchain in the iranbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Iranbit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Iranbit Core will 
...
For an example script see [iranbit-submittx](https://github.com/laanwj/iranbit-submittx).
...
- #5334 `cf87536` libiranbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new iranbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only IranbitAmountField
- #5753 `0f44672` Add iranbit logo to about screen
...
- #5809 `46bfbe7` Add iranbit-cli man page
...
- #5849 `41259ca` contrib/init/iranbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Iranbit-Qt.app to Iranbit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for iranbit-tx)
